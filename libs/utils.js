...
export function buildProgramFromSources(gl, vShaderSrc, fShaderSrc, trfFeedbackVaryings) {
  function getShader(gl, shaderSrc, type) {
    ...
    if (!shaderSrc) {
      console.error("Could not find shader source");
    }
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
      ...
      // also log the source to help debugging
  ...
  // If either shader failed to compile, abort and return null.
  if (!vertexShader || !fragmentShader) {
    console.error('Failed to compile vertex or fragment shader - aborting program build.');
    return null;
  }
